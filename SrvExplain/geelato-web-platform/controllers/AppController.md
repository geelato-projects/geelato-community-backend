...
- Controller：cn.geelato.web.platform.srv.platform.AppController
- BasePath：/app
...
| Summary | Method | Path |
|---|---|---|
| createOrUpdate | POST | `/app/createOrUpdate` |
| get | GET | `/app/get/{id}` |
| isDelete | DELETE | `/app/isDelete/{id}` |
| pageQuery | POST | `/app/pageQuery` |
| query | GET | `/app/query` |
| queryByUser | GET | `/app/queryByUser` |
| queryPermissionByPage | GET | `/app/queryPermissionByPage` |
| queryRolePermissionByPage | GET | `/app/queryRolePermissionByPage` |
| upload | POST | `/app/upload/{fileId}` |
| validate | POST | `/app/validate/{type}` |
...
#### Response
- Java 返回类型：ApiResult<App>

#### curl 示例
```bash
curl -X POST \
  "{{baseUrl}}/app/createOrUpdate" \
  -H "App-Id: {{appId}}" \
...
### queryPermissionByPage

- Method：GET
- Path：`/app/queryPermissionByPage`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/app/queryPermissionByPage" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```

### queryRolePermissionByPage

- Method：GET
- Path：`/app/queryRolePermissionByPage`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/app/queryRolePermissionByPage" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
