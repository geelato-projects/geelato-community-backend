# DevDbConnectController

## 基本信息
- 模块：geelato-web-platform
- Controller：cn.geelato.web.platform.srv.model.DevDbConnectController
...
| Summary | Method | Path |
|---|---|---|
| batchCreate | POST | `/model/connect/batchCreate` |
| createOrUpdate | POST | `/model/connect/createOrUpdate` |
| get | GET | `/model/connect/get/{id}` |
| isDelete | DELETE | `/model/connect/isDelete/{id}` |
| jdbcConnect | POST | `/model/connect/jdbcConnect` |
| pageQuery | POST | `/model/connect/pageQuery` |
| query | GET | `/model/connect/query` |
| refresh | POST | `/model/connect/refresh/{id}` |
| refreshAll | POST | `/model/connect/refreshAll` |
...
- Method：POST
- Path：`/model/connect/batchCreate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/batchCreate" \
  -H "App-Id: {{appId}}" \
...
- Method：POST
- Path：`/model/connect/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/createOrUpdate" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
- Method：DELETE
- Path：`/model/connect/isDelete/{id}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X DELETE \
  "{{baseUrl}}/model/connect/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：POST
- Path：`/model/connect/jdbcConnect`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/jdbcConnect" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/pageQuery" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/refresh/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X POST \
  "{{baseUrl}}/model/connect/refreshAll" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
