...
- Controller：cn.geelato.web.platform.srv.model.DevTableController
...
| Summary | Method | Path |
|---|---|---|
| copy | POST | `/model/table/copy` |
| createOrUpdate | POST | `/model/table/createOrUpdate` |
| get | GET | `/model/table/get/{id}` |
| isDelete | DELETE | `/model/table/isDelete/{id}` |
| pageQuery | POST | `/model/table/pageQuery` |
| query | GET | `/model/table/query` |
| queryDefaultView | GET | `/model/table/queryDefaultView/{entityName}` |
| resetModelFormTable | POST | `/model/table/reset/{tableId}` |
| resetDefaultView | POST | `/model/table/resetDefaultView` |
| upgradesTable | POST | `/model/table/upgradesTable` |
| validate | POST | `/model/table/validate` |
...
#### Response
- Java 返回类型：ApiResult<TableMeta>

#### curl 示例
```bash
curl -X POST \
  "{{baseUrl}}/model/table/copy" \
  -H "App-Id: {{appId}}" \
...
- Method：POST
- Path：`/model/table/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/table/createOrUpdate" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE \
  "{{baseUrl}}/model/table/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
#### Response
- Java 返回类型：ApiResult<List<TableMeta>>

#### curl 示例
```bash
curl -X GET \
  "{{baseUrl}}/model/table/query" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：GET
- Path：`/model/table/queryDefaultView/{entityName}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/model/table/queryDefaultView/{entityName}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：POST
- Path：`/model/table/reset/{tableId}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/table/reset/{tableId}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：POST
- Path：`/model/table/resetDefaultView`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/table/resetDefaultView" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
- Method：POST
- Path：`/model/table/upgradesTable`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/model/table/upgradesTable" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
