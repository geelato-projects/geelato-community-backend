...
- Controller：cn.geelato.web.platform.srv.excel.ExportTemplateController
...
| Summary | Method | Path |
|---|---|---|
| indexTemplate | GET | `/export/template/base64/{id}/{index}` |
| copy | GET | `/export/template/copy/{id}` |
| createOrUpdate | POST | `/export/template/createOrUpdate` |
| generateFile | POST | `/export/template/generateFile/{id}` |
| get | GET | `/export/template/get/{id}` |
| indexTemplate | GET | `/export/template/index/{id}` |
| isDelete | DELETE | `/export/template/isDelete/{id}` |
| pageQuery | GET | `/export/template/pageQuery` |
| query | GET | `/export/template/query` |
...
- Method：GET
- Path：`/export/template/base64/{id}/{index}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/export/template/base64/{id}/{index}" \
  -H "App-Id: {{appId}}" \
...
```bash
curl -X GET \
  "{{baseUrl}}/export/template/copy/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：POST
- Path：`/export/template/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/export/template/createOrUpdate" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
...
- Method：POST
- Path：`/export/template/generateFile/{id}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/export/template/generateFile/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET \
  "{{baseUrl}}/export/template/get/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X GET \
  "{{baseUrl}}/export/template/index/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：DELETE
- Path：`/export/template/isDelete/{id}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X DELETE \
  "{{baseUrl}}/export/template/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：GET
- Path：`/export/template/pageQuery`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/export/template/pageQuery" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
