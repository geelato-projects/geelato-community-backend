...
- Controller：cn.geelato.web.platform.srv.ocr.OcrPdfController
- BasePath：/ocr/pdf
...
| Summary | Method | Path |
|---|---|---|
| createOrUpdate | POST | `/ocr/pdf/createOrUpdate` |
| get | GET | `/ocr/pdf/get/{id}` |
| isDelete | DELETE | `/ocr/pdf/isDelete/{id}` |
| pageQuery | POST | `/ocr/pdf/pageQuery` |
| query | GET | `/ocr/pdf/query` |
| querySelectOptions | GET | `/ocr/pdf/select/group` |
...
```bash
curl -X POST \
  "{{baseUrl}}/ocr/pdf/createOrUpdate" \
  -H "App-Id: {{appId}}" \
