...
- Controller：cn.geelato.web.platform.srv.platform.ScheduleController
- BasePath：/schedule
...
| Summary | Method | Path |
|---|---|---|
| createOrUpdate | POST | `/schedule/createOrUpdate` |
| get | GET | `/schedule/get/{id}` |
| isDelete | DELETE | `/schedule/isDelete/{id}` |
| pageQuery | POST | `/schedule/pageQuery` |
| query | GET | `/schedule/query` |
| changeStatus | GET | `/schedule/status/{id}/{status}` |
| validate | POST | `/schedule/validate` |
...
#### Response
- Java 返回类型：ApiResult<Schedule>

#### curl 示例
```bash
curl -X POST \
  "{{baseUrl}}/schedule/createOrUpdate" \
  -H "App-Id: {{appId}}" \
...
#### Response
- Java 返回类型：ApiResult<List<Schedule>>

#### curl 示例
```bash
curl -X GET \
  "{{baseUrl}}/schedule/query" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：GET
- Path：`/schedule/status/{id}/{status}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/schedule/status/{id}/{status}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
