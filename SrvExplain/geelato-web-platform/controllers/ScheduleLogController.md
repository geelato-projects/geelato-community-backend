...
- Controller：cn.geelato.web.platform.srv.platform.ScheduleLogController
...
| Summary | Method | Path |
|---|---|---|
| createOrUpdate | POST | `/schedule/log/createOrUpdate` |
| get | GET | `/schedule/log/get/{id}` |
| isDelete | DELETE | `/schedule/log/isDelete/{id}` |
| pageQuery | POST | `/schedule/log/pageQuery` |
| query | GET | `/schedule/log/query` |
...
- Method：POST
- Path：`/schedule/log/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
#### Response
- Java 返回类型：ApiResult<ScheduleLog>

#### curl 示例
```bash
curl -X POST \
  "{{baseUrl}}/schedule/log/createOrUpdate" \
  -H "App-Id: {{appId}}" \
...
```bash
curl -X DELETE \
  "{{baseUrl}}/schedule/log/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X POST \
  "{{baseUrl}}/schedule/log/pageQuery" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
#### Response
- Java 返回类型：ApiResult<List<ScheduleLog>>

#### curl 示例
```bash
curl -X GET \
  "{{baseUrl}}/schedule/log/query" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
