# StaticSiteController

## 基本信息
- 模块：geelato-web-platform
- Controller：cn.geelato.web.platform.srv.site.StaticSiteController
- BasePath：/site/statics
...
| Summary | Method | Path |
|---|---|---|
| batchDelete | POST | `/site/statics/batchDelete` |
| createFolder | POST | `/site/statics/createFolder` |
| createOrUpdate | POST | `/site/statics/createOrUpdate` |
| download | POST | `/site/statics/download` |
| get | GET | `/site/statics/get/{id}` |
| isDelete | DELETE | `/site/statics/isDelete/{id}` |
| pageQuery | POST | `/site/statics/pageQuery` |
| query | GET | `/site/statics/query` |
| queryFile | POST | `/site/statics/queryFile` |
| treeNode | POST | `/site/statics/treeNode` |
| updateName | POST | `/site/statics/updateName` |
| upload | POST | `/site/statics/upload` |
| validate | POST | `/site/statics/validate` |
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/batchDelete" \
  -H "App-Id: {{appId}}" \
...
- Method：POST
- Path：`/site/statics/createFolder`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/createFolder" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
- Method：POST
- Path：`/site/statics/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/createOrUpdate" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE \
  "{{baseUrl}}/site/statics/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/pageQuery" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/queryFile" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST \
  "{{baseUrl}}/site/statics/updateName" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
  -H "Content-Type: application/json" \
