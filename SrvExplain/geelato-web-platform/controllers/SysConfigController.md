...
- Controller：cn.geelato.web.platform.srv.platform.SysConfigController
- BasePath：/sys/config
...
| Summary | Method | Path |
|---|---|---|
| createOrUpdate | POST | `/sys/config/createOrUpdate` |
| get | GET | `/sys/config/get/{id}` |
| getValue | GET | `/sys/config/getValue/{key}` |
| isDelete | DELETE | `/sys/config/isDelete/{id}` |
| pageQuery | POST | `/sys/config/pageQuery` |
| query | GET | `/sys/config/query` |
| validate | POST | `/sys/config/validate` |
...
- Method：POST
- Path：`/sys/config/createOrUpdate`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/sys/config/createOrUpdate" \
  -H "App-Id: {{appId}}" \
...
```bash
curl -X GET \
  "{{baseUrl}}/sys/config/getValue/{key}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
```bash
curl -X DELETE \
  "{{baseUrl}}/sys/config/isDelete/{id}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
#### Response
- Java 返回类型：ApiResult<List<SysConfig>>

#### curl 示例
```bash
curl -X GET \
  "{{baseUrl}}/sys/config/query" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
