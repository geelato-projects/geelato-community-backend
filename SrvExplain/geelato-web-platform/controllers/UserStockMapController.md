# UserStockMapController

## 基本信息
- 模块：geelato-web-platform
- Controller：cn.geelato.web.platform.srv.security.UserStockMapController
- BasePath：/security/user/stock
- 条件装配：designtime（@ApiRestController）
- 分类：
- 源码：file:///D:/geelato/geelato-enterprise/geelato-community/geelato-web-platform/src/main/java/cn/geelato/web/platform/srv/security/UserStockMapController.java#L26-L127

## 接口列表

| Summary | Method | Path |
|---|---|---|
| create | POST | `/security/user/stock/create` |
| pageQuery | POST | `/security/user/stock/pageQuery` |
| query | GET | `/security/user/stock/query` |
| remove | DELETE | `/security/user/stock/remove/{stockId}` |
| total | GET | `/security/user/stock/total` |
...
- Method：POST
- Path：`/security/user/stock/create`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/security/user/stock/create" \
  -H "App-Id: {{appId}}" \
...
- Method：POST
- Path：`/security/user/stock/pageQuery`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X POST \
  "{{baseUrl}}/security/user/stock/pageQuery" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：GET
- Path：`/security/user/stock/query`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/security/user/stock/query" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：DELETE
- Path：`/security/user/stock/remove/{stockId}`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X DELETE \
  "{{baseUrl}}/security/user/stock/remove/{stockId}" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
...
- Method：GET
- Path：`/security/user/stock/total`
- Produces：
- Consumes：
- 鉴权：需要 Authorization
...
```bash
curl -X GET \
  "{{baseUrl}}/security/user/stock/total" \
  -H "App-Id: {{appId}}" \
  -H "Tenant-Code: {{tenantCode}}" \
  -H "Authorization: {{authorization}}" \
```
