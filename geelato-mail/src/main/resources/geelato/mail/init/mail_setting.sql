-- geelato-mail mail_setting 表（提取自 fms db/migration/V130__create_mail_p3_settings_tables.sql，catalog 逻辑分组 mail，表名不加库前缀）
...
CREATE TABLE IF NOT EXISTS `mail_setting` (
  ...
  `setting_key` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '设置键（general 等）',
  `setting_value` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '设置值（JSON）',
  ...
  INDEX `idx_user_key` (`user_id` ASC, `setting_key` ASC, `del_status` ASC) USING BTREE,
  INDEX `idx_tenant_status` (`tenant_code` ASC, `del_status` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '邮件用户设置表' ROW_FORMAT = Dynamic;
