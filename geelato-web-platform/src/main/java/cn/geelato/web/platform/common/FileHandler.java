...
public class FileHandler {
    ...
    public void updateAttachment(Attachment attachment) {
        accessoryHandler.updateAttachment(attachment);
    }
    ...
    public Attachment uploadCloud(File file, String name, FileParam param) throws IOException {
        OSSResult ossResult = requireFileHelper().putFile(name, FileUtils.openInputStream(file));
    ...
    public Attachment uploadCloudFromLocal(Attachment localSource) throws IOException {
        OSSResult ossResult = requireFileHelper().putFile(localSource.getName(), FileUtils.openInputStream(new File(localSource.getPath())));
        if (ossResult.getSuccess() == null || ossResult.getSuccess()) {
            if (ossResult.getOssFile() != null) {
    ...
    public void download(String id, boolean isPdf, boolean isPreview, boolean isThumbnail, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (Strings.isNotBlank(attachment.getObjectId())) {
            OSSResult ossResult = requireFileHelper().getFile(attachment.getPath());
    ...
    public void delete(String id, Boolean isRemoved, boolean deleteOss) {
        if (deleteOss) {
            ...
            if (attachment != null && Strings.isNotBlank(attachment.getObjectId()) && Strings.isNotBlank(attachment.getPath())) {
                try {
                    OSSResult ossResult = requireFileHelper().removeFile(attachment.getPath());
                    if (ossResult.getSuccess() != null && ossResult.getSuccess()) {
