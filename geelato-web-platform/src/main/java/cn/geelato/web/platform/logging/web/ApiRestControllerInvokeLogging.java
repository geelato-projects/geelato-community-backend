...
import cn.geelato.web.platform.srvlog.usage.ApiEndpointUsageCollector;
...
public class ApiRestControllerInvokeLogging implements HandlerInterceptor, ResponseBodyAdvice<Object> {
    private static final String HEADER_CALL_SCOPE = "X-Gl-Call-Scope";
    ...
    private final ApiEndpointUsageCollector usageCollector;
    private final ObjectMapper objectMapper;

    public ApiRestControllerInvokeLogging(SrvLogService srvLogService, ApiEndpointUsageCollector usageCollector, ObjectMapper objectMapper) {
    ...
    public void afterCompletion(HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull Object handler, @Nullable Exception ex) {
        ...
        String callScope = resolveCallScope(request);
        r.setCallScope(callScope);
        ...
        usageCollector.record(methodKey, callScope);
        ...
        if (r.getCallScope() != null && !r.getCallScope().isBlank()) {
            requestLog.put("callScope", r.getCallScope());
        }
    ...
    private String resolveCallScope(HttpServletRequest request) {
        String callScope = request.getHeader(HEADER_CALL_SCOPE);
        if (callScope == null) {
            return null;
        }
