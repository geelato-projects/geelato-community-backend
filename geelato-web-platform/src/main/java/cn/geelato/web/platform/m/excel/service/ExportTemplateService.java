...
public class ExportTemplateService extends BaseService {
    public static final String[] EXPORT_META_HEADER = {"占位符", "变量", "列表变量", "常量值", "表达式", "值类型", "取值计算方式", "来源于列表", "合并单元格", "合并唯一约束", "图片", "图片宽度cm", "图片高度cm", "图片来源", "条形码编码", "输入值格式", "输出值格式", "导出批注", "备注", "动态列表", "动态类型"};
    ...
    private void exportMetaSheet(XSSFWorkbook workbook, String sheetName, List<PlaceholderMeta> metas) {
        ...
        for (int i = 0; i < metas.size(); i++) {
            ...
            ExcelXSSFUtils.setCell(dRow, 18, cellStyle, metas.get(i).getRemark());
            ExcelXSSFUtils.setCell(dRow, 19, cellStyle, metas.get(i).isIsDynamic());
            ExcelXSSFUtils.setCell(dRow, 20, cellStyle, metas.get(i).getDynamicType());
        }
        // 设置列宽
