 ...
 */
@DesignTimeApiRestController("/model/table/import")
@Slf4j
public class DevTableImportController extends BaseController {
     ...
     * @param table   表对�?
     * @param columns 字段数组
     * @param checks  检查数组，可为�?
     * @return 通过返回null，失败返回错误信�?
     */
    ...
    /**
     * 构建�?platform_dev_table 为根的保存结构，子实体使�?#platform_dev_column / #platform_dev_table_check
     * 并执行审计字段填充与布尔数值化（true/false -> 1/0�?
     *
     * @param table       表对�?
     * @param columns     字段数组
     * @param checks      检查数�?
     * @param preserveId  是否保留外部主键（使�?forceId�?
     * @param user        当前用户
     * @param now         当前时间
     * @return 根保存结构的JSON字符�?
     */
    ...
    /**
     * 将布尔型及其字符串表示统一转换为数值（1/0），并递归处理对象或数�?
     *
     * @param v 输入�?
     * @return 规范化后的�?
     */
     ...
     * 请求体：原始JSON字符�?
     * 要求：table/columns/checks 所有记录必须包�?id
     * 行为：校�?�?构建命令树（保留forceId）→ 规则保存 �?可选DDL创建/更新 �?刷新缓存 �?返回计数
     * 参数：ddl（默认true）是否在导入成功后立即创�?更新数据库表结构
     */
     ...
     * 请求体：原始JSON字符�?
     * 要求：table/columns/checks 所有记录禁止包�?id
     * 行为：校�?�?构建命令树（不设置forceId）→ 规则保存 �?可选DDL创建/更新 �?刷新缓存 �?返回计数
     * 参数：ddl（默认true）是否在导入成功后立即创�?更新数据库表结构
     */
     ...
     * 这个方法主要用于设置RuleService对象，以便在类内部使�?
     *
     * @param ruleService 规则服务对象，用于执行规则操�?
     */
    @Autowired
