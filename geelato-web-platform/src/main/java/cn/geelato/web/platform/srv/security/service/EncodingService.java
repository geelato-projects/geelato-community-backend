...
import cn.geelato.web.platform.srv.security.entity.Encoding;
import cn.geelato.meta.EncodingLog;
...
public class EncodingService extends BaseService {
    ...
    public List<Object> querySerialsByEncodingLog(Encoding encoding) {
        ...
        if (Strings.isNotBlank(encoding.getDateType())) {
            logParams.put("exampleDate", formatDate(encoding.getDateType()));
        }
        ...
        for (EncodingItem item : itemList) {
            if (EncodingItemTypeEnum.CONSTANT.getValue().equals(item.getItemType())) {
                ...
                if (Strings.isNotBlank(item.getDateType())) {
                    try {
                        String date = formatDate(item.getDateType());
                        examples.add(date);
     ...
     * 格式化日期
     * <p>
     * 根据提供的日期格式模式，将当前日期格式化为指定的字符串格式。
     * 支持yyM和yyyyM格式，分别表示年和年月。
     *
     * @param pattern 日期格式模式，例如"yyM"或"yyyyM"
     * @return 格式化后的日期字符串
     */
    private String formatDate(String pattern) {
        ...
        switch (pattern) {
            case "yyM":
                ...
                return yy + DateUtils.hexMonth(now.getMonthValue());
            case "yyyyM":
                String yyyy = now.format(DateTimeFormatter.ofPattern("yyyy"));
                return yyyy + DateUtils.hexMonth(now.getMonthValue());
            default:
