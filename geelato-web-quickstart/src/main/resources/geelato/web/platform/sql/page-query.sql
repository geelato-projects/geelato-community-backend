...
-- @sql page_query_platform_app_r_connect
SELECT
    p1.id,
    p1.app_id as appId,
    p1.tenant_code as tenantCode,
    p1.connect_id as connectId,
    p1.update_at as updateAt,
    p1.updater,
    p1.updater_name as updaterName,
    p1.create_at as createAt,
    p1.creator,
    p1.creator_name as creatorName,
    p2.db_schema as connectSchema,
    p2.db_hostname_ip as connectHostName,
    p2.db_name as connectName,
    p2.db_type as connectType,
    p2.db_user_name as connectUserName,
    p2.db_connect_name as connectTitle,
    p2.db_port as connectPort,
    p2.enable_status as connectEnableStatus,
    p3.logo as appLogo,
    p3.name as appName,
    p3.code as appCode,
    p3.type as appType,
    ...
    p3.seq_no as appSeqNo,
    p3.description as appDescription
FROM platform_app_r_connect p1
LEFT JOIN platform_dev_db_connect p2 ON p2.id = p1.connect_id
LEFT JOIN platform_app p3 ON p3.id = p1.app_id
WHERE 1=1 AND p1.del_status = 0
@if $.connectId!=null&&$.connectId!=''
  AND p1.connect_id = '$.connectId'
@/if
@if $.appId!=null&&$.appId!=''
  AND p1.app_id = '$.appId'
@/if
...
@if $.connectName!=null&&$.connectName!=''
  AND p2.db_name like '%$.connectName%'
@/if
@if $.connectTitle!=null&&$.connectTitle!=''
  AND p2.db_connect_name like '%$.connectTitle%'
@/if
@if $.connectType!=null&&$.connectType!=''
  AND p2.db_type = '$.connectType'
@/if
@if $.connectEnableStatus!=null&&$.connectEnableStatus!=''
  AND p2.enable_status = '$.connectEnableStatus'
@/if
@if $.appName!=null&&$.appName!=''
  AND p3.name like '%$.appName%'
@/if
