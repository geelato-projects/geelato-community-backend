...
It extends the runtime baseline with design-time features, so it should remain distinct from pure runtime deployment.
...
- keep `geelato.web.platform.design-time.enabled` aligned with the image responsibility
