...
本文档介绍接入的前提条件、交互时序，以及如何获取 Token 和用户数据的核心步骤。

## 1. 接入前提：申请应用凭证

在开始集成之前，外部业务系统管理员需要向统一认证中心管理员申请应用接入凭证。你需要获取并妥善保管以下信息：

- **`client_id`**：应用的唯一标识。
- **`client_secret`**：应用的安全秘钥（**绝对不能**暴露给前端或浏览器，必须在后端代码中安全存放）。
...
- `client_id`：你申请的 `client_id`
- `redirect_uri`：你申请的合法的回调地址
- `state`：（强烈推荐）防止 CSRF 攻击的随机字符串，业务系统需自行验证该字段

**回调结果**：
用户在认证中心登录成功后，认证中心会将浏览器重定向回你的 `redirect_uri`，并在 URL 中附带 `code` 参数：
...
拿到 `access_token` 后，业务系统后端需要调用用户信息接口获取当前登录用户的真实身份。
...
> **非常重要**：
> 业务系统必须从返回结果的 `data.user` 中提取出用户数据。然后，根据提取出的身份标识（如 `loginId` 或 `user.id`），与业务系统自身数据库中的用户体系进行映射。映射成功后，再建立业务系统自己的用户会话（如写入本地 Session、下发业务系统自己的 Cookie 等）。

## 4. 常见问题

- **`client_secret` 不慎泄露了怎么办？**
  必须立即联系统一认证中心管理员重置秘钥，否则恶意第三方可能会伪造你的应用骗取用户 Token。

- **Token 过期了怎么办？**
  如果授权时下发了 `refresh_token`，可以通过调用 `/oauth2/token`（将 `grant_type` 设为 `refresh_token`）接口在服务端进行无感刷新；如果未提供刷新 Token 或刷新也失败了，则必须清理本地会话并要求用户重新登录。
