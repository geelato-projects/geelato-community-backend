---
title: ORM 注解说明
sidebar_label: ORM 注解说明
---

# ORM 注解说明

本页说明 Geelato Framework 中推荐的 ORM 注解及其在实体声明中的职责边界。
...
```java
@Entity(name = "Org", table = "platform_org")
public class Demo {
    private String name;
    private String code;
    private String pid;
...
- `name` 和 `table` 是两个概念：前者是 ORM 里的实体名，后者是数据库表名
- 类中的普通属性默认参与字段映射
- 列名默认按属性名做驼峰转蛇形转换
...
```java
@Entity(name = "FileInfo", table = "platform_file")
@Title(title = "文件")
...
```java
@Entity(name = "Permission", table = "platform_permission")
